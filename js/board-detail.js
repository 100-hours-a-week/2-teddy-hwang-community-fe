  ...
  commentCheckBtn.addEventListener('click', async () => {
    ...
    try {
      const response = await apiDelete(
        `${address}/api/comments/${deleteCommentId}`,
      );
...
const displayPost = post => {
  ...
  post.comments.forEach(comment => {
    ...
    replyItem
      .querySelector('.reply-info-wrap')
      .insertBefore(usernameSpan, replyItem.querySelector('.reply-created-at'));
...
const handleComment = async () => {
  ...
  commentList.addEventListener('click', e => {
    if (e.target.classList.contains('modify-reply-btn')) {
      ...
      const replyContent = e.target
        .closest('.reply-item')
        .querySelector('.reply-content').textContent;
    ...
    try {
      ...
      if (commentId) {
        ...
        response = await apiPatch(
          `${address}/api/comments/${commentId}`,
      ...
      if (!response.response.ok) {
        throw new Error(
          commentId ? '댓글 수정에 실패했습니다.' : '댓글 작성에 실패했습니다.',
        );
      }
      await loadBoardData();
...
const handleLike = async () => {
  ...
  const loadLikeStatus = async () => {
    ...
    try {
      const response = await apiGet(
        `${address}/api/posts/${postId}/like?userId=${userId}`,
      );
  ...
  const updateLikeUI = () => {
    ...
    likeBtn.animate(
      isLiked
        ? [
            { transform: 'scale(1)', marginRight: '0' },
            { transform: 'scale(1.2)', marginRight: likeSpacing },
          ]
        : [
            { transform: 'scale(1.2)', marginRight: likeSpacing },
            { transform: 'scale(1)', marginRight: '0' },
