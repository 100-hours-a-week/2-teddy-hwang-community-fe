...
const handleEmailValidation = async (emailInput, helpertext) => {
  ...
  try {
    const response = await apiGetNoHeader(
      `${address}/api/users/email/${email}`,
    );
...
const handleNicknameValidation = async (nicknameInput, helpertext) => {
  ...
  try {
    const response = await apiGetNoHeader(
      `${address}/api/users/signup/nickname/${nickname}`,
    );
    if (!response.data.data) {
...
const handleSubmit = async event => {
  ...
  try {
    const result = await apiPostFormDataNoHeader(
      `${address}/api/users`,
      formData,
    );
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  emailInput.addEventListener('blur', () => {
    handleEmailValidation(
      emailInput,
      document.getElementById('email-helpertext'),
    );
  });
  ...
  nicknameInput.addEventListener('blur', () => {
    handleNicknameValidation(
      nicknameInput,
      document.getElementById('nickname-helpertext'),
    );
  });
